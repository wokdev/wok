...
pub fn test_auth<W: Write>(
    ...
    if let Some(tracking) = repo.tracking_branch(branch_name)? {
            ...
            Ok(mut remote) => {
                ...
                match remote.connect_auth(
                    ...
                    Err(e) => {
                        ...
                        writeln!(stdout, "2. Verify SSH_AUTH_SOCK is set: echo $SSH_AUTH_SOCK")?;
                        writeln!(stdout, "3. Test SSH connection: ssh -T git@<hostname>")?;
        ...
        writeln!(stdout, "No tracking branch configured for '{}'", branch_name)?;
        writeln!(stdout)?;
        writeln!(stdout, "To set up tracking, run:")?;
        writeln!(stdout, "  git branch --set-upstream-to=<remote>/<branch> {}", branch_name)?;
    }
